...
def get_parser():
    ...
    parser.add("--config", is_config_file=True, help="DAN/DAN.yaml",default='DSAN/DSAN.yaml')
    # parser.add("--config", is_config_file=True, help="DAN/DAN.yaml",default='DeepCoral/DeepCoral.yaml')
    ...
    # parser.add_argument('--data_dir', type=str, default='H:/Transfer learning/office31')
    parser.add_argument('--data_dir', type=str, default='./office31')
    parser.add_argument('--src_domain', type=str, default='amazon')
    parser.add_argument('--tgt_domain', type=str, default='dslr')
...
def get_optimizer(model, args):
    ...
    optimizer = torch.optim.SGD(params, lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay,
                                nesterov=False)
    # optimizer = AdaBelief(params, lr=args.lr, weight_decay=args.weight_decay)
    return optimizer
...
def test(model, target_test_loader, args):
    ...
    return acc, test_loss.avg, target_features
...
def train(source_loader, target_train_loader, target_test_loader, model, optimizer, lr_scheduler, args):
    ...
    for e in range(1, args.n_epoch + 1):
        ...
        # Test
        stop += 1
        test_acc, test_loss, target_features = test(model, target_test_loader, args)
        adistance = cdist(source_features, target_features, metric='euclidean').mean() 
        adistances.append(adistance)
        info += ', A-distance: {:.4f}'.format(adistance)
    ...
    np_adistances = np.array(adistances)
    np.savetxt('./adistances.csv', np_adistances, delimiter=',', fmt='%.6f')
